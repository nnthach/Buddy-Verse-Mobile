...
export default function ChooseInterestGroup() {
  const { data: interestList, loading } = useFetchList(getInterestListAPI);
  const { setMatchGroupForm, matchGroupForm } = useContext(ChatGroupContext);

  const handleAddInterestList = (item) => {
    setMatchGroupForm((prev) => {
  ...
  const handleNavigate = () => {
    if (matchGroupForm.interestIds.length < 3) {
      Toast.show({
        type: "error",
      ...
      router.replace({
        pathname: "/(stack)/match/matchGroupLoading",
        params: {
          label: "Đang tìm nhóm...",
        },
      });
      ...
      >
              ...
              interestList.map((item) => (
                <TouchableOpacity
                  ...
                  className={`w-[30%] aspect-square rounded-xl items-center justify-center p-3 mb-4 ${
                    matchGroupForm.interestIds.includes(item.interestId)
                      ? "bg-yellow-primary"
                      : "bg-gray-five"
                  }`}
                >
        ...
        <View className="gap-4">
          ...
          <TouchableOpacity
            ...
            disabled={matchGroupForm.interestIds.length < 3}
            className={`h-14 rounded-xl items-center justify-center ${
              matchGroupForm.interestIds.length >= 3
                ? "bg-yellow-primary"
                : "bg-gray-five"
            }`}
          >
