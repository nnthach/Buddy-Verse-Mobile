...
export default function BuddyScreen() {
  ...
  const [matchJoinDataForm, setMatchJoinDataForm] = useState({
    accountId: userId,
    roomType: "",
    interestIds: [],
  });

  const [isLoadingMatchJoin, setIsLoadingMatchJoin] = useState(false);
  ...
  const handleJoinMatch = async () => {
    console.log("match join form", matchJoinDataForm);
    if (matchJoinDataForm.interestIds.length < 3) {
      Toast.show({
        type: "error",
    ...
    setIsLoadingMatchJoin(true);
    try {
      const res = await matchJoinAPI(matchJoinDataForm);
      console.log("match join res", res);
      console.log("match join res data", res.data);

      setIsLoadingMatchJoin(false);
    } catch (error) {
  ...
  return (
    <>
          ...
          <View className="gap-6">
            ...
            <View className="flex-row justify-between items-center w-full gap-3">
              <TouchableOpacity
                ...
                className={`${matchJoinDataForm.roomType == "individual" ? "bg-purple-primary" : "bg-purple-primary/50"} h-28 flex-1 rounded-xl`}
              >
              ...
              <TouchableOpacity
                ...
                className={`${matchJoinDataForm.roomType == "group" ? "bg-purple-primary" : "bg-purple-primary/50"} h-28 flex-1 rounded-xl`}
              >
                <Text>group</Text>
              </TouchableOpacity>
            </View>
                ...
                <View className="flex-row flex-wrap gap-4 my-auto">
                  {interestList.map((item) => (
                    <TouchableOpacity
                      ...
                      className={`rounded-2xl h-9 items-center justify-center ${matchJoinDataForm.interestIds.includes(item.interestId) ? "bg-purple-third" : "bg-purple-third/50"}`}
                    >
            ...
            <View className="">
              ...
              <View className="w-full flex-row items-center justify-between">
                ...
                <View className="flex-row items-center">
                  <Text className="text-purple-primary/50 text-lg">
                    See more
                  </Text>
                  <MaterialIcons
      ...
      {isLoadingMatchJoin && <LoadingCustom label={"Matching in process..."} />}
    </>
  );
}
